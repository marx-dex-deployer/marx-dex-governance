...
import MarXDEXV2Factory from '@marx-dex/marx-dex-v2-core/build/MarXDEXV2Factory.json'
import MarXDEXV2Pair from '@marx-dex/marx-dex-v2-core/build/MarXDEXV2Pair.json'
...
describe('scenario:FeeTo', () => {
  ...
  beforeEach('deploy MarXDEX v2', async () => {
    factory = await deployContract(wallet, MarXDEXV2Factory, [wallet.address])
  })
  ...
  describe('tokens', () => {
    ...
    beforeEach('make test tokens', async () => {
      const { timestamp: now } = await provider.getBlock('latest')
      const token0 = await deployContract(wallet, MarX, [wallet.address, constants.AddressZero, now + 60 * 60])
      tokens.push(token0)
      const token1 = await deployContract(wallet, MarX, [wallet.address, constants.AddressZero, now + 60 * 60])
      tokens.push(token1)
    })
    ...
    beforeEach('create fee liquidity', async () => {
      ...
      pair = new Contract(pairAddress, MarXDEXV2Pair.abi).connect(wallet)
