...
export async function governanceFixture(
  ...
  // deploy MARX, sending the total supply to the deployer
  const { timestamp: now } = await provider.getBlock('latest')
  const timelockAddress = Contract.getContractAddress({ from: wallet.address, nonce: 1 })
  const marx = await deployContract(wallet, MarX, [wallet.address, timelockAddress, now + 60 * 60])
  ...
  const governorAlpha = await deployContract(wallet, GovernorAlpha, [timelock.address, marx.address])
  expect(governorAlpha.address).to.be.eq(governorAlphaAddress)

  return { marx, timelock, governorAlpha }
}
